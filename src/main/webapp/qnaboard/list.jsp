...
<%
	...
	QnaboardVO article = alist.get(i);	
	replycount = rdao.getArticleCount(article.getNum());
%>
   	<li class="b-content">
      <span class="col1"><%=number--%></span>
      <span class="col2 board_title"><a href="content.jsp?num=<%=article.getNum()%>&pageNum=<%=currentPage%>"><%=article.getSubject()%></a>
...
<%
		if(startPage > 10){
%>
				<a class="pageNumBtn"  href="qna.jsp?pageNum=<%= startPage-pageBlock %>">이전</a>
<%	
		}
		for(int i = startPage; i<=endPage; i++){
			if(currentPage == i){
%>
				<a class="pageNumBtn current" href="qna.jsp?pageNum=<%=i%>"><%= i %></a>
<%
			}else{
%>
				<a class="pageNumBtn" href="qna.jsp?pageNum=<%=i%>"><%= i %></a>
<%
			}
		}
		if(endPage < pageCount){
%>
				<a class="pageNumBtn" href="qna.jsp?pageNum=<%= startPage+pageBlock %>">다음</a>
<%	
		}
	}
//-----------------------------------	
