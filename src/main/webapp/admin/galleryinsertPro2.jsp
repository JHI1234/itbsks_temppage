	...
	if(managerId==null || managerId.equals("")) {
		...
		try{
		   ...
		   out.println("업로드 성공");
			...
			String fileName = multi.getFilesystemName("gimage");

			int pos = fileName.lastIndexOf(".");
			String _fileName = fileName.substring(0, pos);
			String ext = fileName.substring(pos);
			   
		   //filename = multi.getFilesystemName("gimage")+ strdate; //서버에 저장된 파일 이름
		   filename = _fileName +"_"+ strdate + ext; //서버에 저장된 파일 이름
		     
			System.out.println(fileName);
			File oldFile = new File(saveFolder + fileName);
			File newFile = new File(saveFolder + filename);
