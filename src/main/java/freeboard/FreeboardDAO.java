...
public class FreeboardDAO {
	...
	// 게시판 글 삽입 메서드
	public void insertFreeboard(FreeboardVO article) {
		Connection conn = null;			// 데이터베이스 연결할 객체
		PreparedStatement pstmt = null;	// query 실행 객체
	...
	public int getArticleCount() {
		...
		try {
			...
			String sql = "select count(*) from freeboard";	// 쿼리 구문
			pstmt  = conn.prepareStatement(sql);	// 3. sql을 실행하기 위한 객체 생성하기
			...
			e.printStackTrace();
			System.out.println("Freeboard 테이블의 자료 수 검색에 실패했습니다.");
		}finally{	// 5. 자원 해제
			if(rs != null) try{rs.close();}catch(SQLException se){}		// 예외 처리 해줘야함
			if(pstmt != null) try{pstmt.close();}catch(SQLException se){}
			if(conn != null) try{conn.close();}catch(SQLException se){}	// 연결 해제
	...
	public int getMemberArticleCount(String id) {
		...
		try {
			...
			String sql = "select count(*) from freeboard where writer=?";	// 쿼리 구문
			pstmt  = conn.prepareStatement(sql);	// 3. sql을 실행하기 위한 객체 생성하기
			...
			e.printStackTrace();
			System.out.println("Freeboard 테이블에서 특정 사용자가 작성한 게시글 수 검색에 실패했습니다.");
		}finally{	// 5. 자원 해제
			if(rs != null) try{rs.close();}catch(SQLException se){}		// 예외 처리 해줘야함
			if(pstmt != null) try{pstmt.close();}catch(SQLException se){}
			if(conn != null) try{conn.close();}catch(SQLException se){}	// 연결 해제
	...
	public List<FreeboardVO> getArticles(int start, int end){
		...
		PreparedStatement pstmt = null;	// query 실행 객체
		ResultSet rs = null;			// select문 실행 결과를 저장하는 객체
		List<FreeboardVO> alist = null;		// return 값
		
		try {
			conn = getConnection();
			                                                    // 게시글 개수 제한
			String sql = "select * from freeboard order by num desc limit ?, ?;";	// 쿼리 구문
			pstmt  = conn.prepareStatement(sql);	// 3. sql을 실행하기 위한 객체 생성하기
			...
			// ↓ result set의 next가 존재할 경우 값이 저장됨
			if(rs.next()){	// 없으면 0이 반환됨
				alist = new ArrayList<FreeboardVO>(end);	// 배열의 개수지정 = 한 페이지에 표시할 게시물의 개수 지정
				do {
			...
			e.printStackTrace();
			System.out.println("Freeboard 테이블의 자료 검색에 실패했습니다.");
		}finally{	// 5. 자원 해제
			if(rs != null) try{rs.close();}catch(SQLException se){}		// 예외 처리 해줘야함
			if(pstmt != null) try{pstmt.close();}catch(SQLException se){}
			if(conn != null) try{conn.close();}catch(SQLException se){}	// 연결 해제
	...
	public List<FreeboardVO> getMemberArticles(String id, int start, int end){
		...
		try {
			conn = getConnection();
			                                                    // 게시글 개수 제한
			String sql = "select * from freeboard where writer=? order by num desc limit ?, ?;";	// 쿼리 구문
			pstmt  = conn.prepareStatement(sql);	// 3. sql을 실행하기 위한 객체 생성하기
			pstmt.setString(1, id);
			...
			e.printStackTrace();
			System.out.println("Freeboard 테이블에서 특정 사용자가 작성한 게시글 검색에 실패했습니다.");
		}finally{	// 5. 자원 해제
			if(rs != null) try{rs.close();}catch(SQLException se){}		// 예외 처리 해줘야함
			if(pstmt != null) try{pstmt.close();}catch(SQLException se){}
			if(conn != null) try{conn.close();}catch(SQLException se){}	// 연결 해제
	...
	public FreeboardVO getArticleUpdate(int num){
		...
		PreparedStatement pstmt = null;	// query 실행 객체
		ResultSet rs = null;			// select문 실행 결과를 저장하는 객체
		FreeboardVO article = null;		// return 값
		
		try {
			conn = getConnection();
			
			String sql = "select * from freeboard where num=?;";	// 쿼리 구문
	...
	public FreeboardVO getArticle(int num){
		...
		try {
			...
			String sql = "update freeboard set readcount=readcount+1 where num=?;";	// 쿼리 구문
			pstmt  = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.executeUpdate(); // 4. sql query 실행 -  개수 반환
			
			sql = "select * from freeboard where num=?;";	// 쿼리 구문
			pstmt  = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();	// 4. sql query 실행 -  개수 반환
			...
			e.printStackTrace();
			System.out.println("Freeboard 테이블의 상세보기 레코드 검색에 실패했습니다.");
		}finally{	// 5. 자원 해제
			if(rs != null) try{rs.close();}catch(SQLException se){}		// 예외 처리 해줘야함
			if(pstmt != null) try{pstmt.close();}catch(SQLException se){}
			if(conn != null) try{conn.close();}catch(SQLException se){}	// 연결 해제
	...
	public int updateFreeboard(FreeboardVO article) {	
		...
		try {
			...
			String sql = "select passwd from freeboard where num=?";	// 쿼리 구문
			pstmt  = conn.prepareStatement(sql);	// 3. sql을 실행하기 위한 객체 생성하기
